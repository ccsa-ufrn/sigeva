...
class ReportModule extends Module {
  ...
  getAllEnrollments() {
    const usersIds = this.event.eventObject.ofRelationships.map((r) => ({
      user: r.user,
    }));
    return new Promise((resolve, reject) => {
      ModuleModel.populate(
        usersIds,
        ...
        (err, users) => {
          ...
          // Add roles
          const userWithRoles = users
            .filter((usr) => usr.user)
            .map((usr) => {
              const user = usr.user;
              let roles = this.event.getUserRelationships(
                String(usr.user._id)
              ).roles;
  ...
  getUserReport(uId) {
    ...
    return new Promise((resolve, reject) => {
      user
        .loadById(uId)
        .then(() => {
          user.toFormatedUser("cpf institution phone").then((formatedUser) => {
            ...
            // mount activities
            this.event.getModule("submission").then((submissionMdle) => {
              submissionMdle
                .getObjectsByUserId("article", uId)
                .then((userArticles) => {
                  submissionMdle
                    .getObjectsByUserId("poster", uId)
                    .then((userPosters) => {
                      submissionMdle
                        .getObjectsByUserId("teachingcases", uId)
                        .then((userTCases) => {
                          this.event
                            .getModule("activities")
                            .then((activitiesMdle) => {
                              activitiesMdle
                                .getAllObjectsUserEnrolled(uId)
                                .then((userActivities) => {
                                  resolve({
                                    _id: formatedUser.id,
  ...
  act(user, roles, body, entitySlug, subaction) {
    ...
    const seeAllEnrollments = permissions.find(
      (perm) => perm.action === "see_all_enrollments"
    );
